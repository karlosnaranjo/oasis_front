...
const {
  presolped: {
    presolpedDetalle: { createTitle, updateTitle }
  }
} = masterMessages;
...
function PresolpedDetalleForm({
  ...
  console.log('al entrar en el MODAL '+presolped_id);
  const [loading, setLoading] = useState(true);
  const [formState, setFormState] = useState(initialValues);
  const main_id = presolped_id;

  const titleForm = `${!id ? createTitle : updateTitle}`;

  const loadInitForm = async () => {
    const paramas = {
      url: endPoints.presolped.presolpedDetalle.initForm,
      data: { id: id || '' }
    };
  ...
  const init = async () => {
    try {
      const { presolpedDetalle, material, maquina, motivoSolicitud, centroCosto } = await loadInitForm();
      ...
      setFormState({
        presolped_id: presolped_id || main_id,
        cantidad: cantidad || 0,
  ...
  const mapValues = (values) => {
    const {  presolped_id, cantidad, estado, material_id, maquina_id, motivo_solicitud_id, centro_costo_id } = values;
  ...
  const onSubmit = async (formValues) => {
    ...
    const params = {
      url: `${endPoints.presolped.presolpedDetalle.base}${id ? `/${id}` : ''}`,
      data: dataValues
    };
              ...
              <Form>
                <Grid container direction="row" spacing={1}>
                  <Grid item xs={12} md={12} xl={12}>
                    <Field label="presolped_id" name="presolped_id" component={TextBase}  style={ {display:'none'}} />
                    <Field label="Material" name="material_id" component={SelectBase} items={materialesList}/>
                  </Grid>
                  <Grid item xs={6} md={6} xl={6}>
                    <Field label="Máquina" name="maquina_id" component={SelectBase} items={maquinasList} />
                  </Grid>
                  <Grid item xs={6} md={6} xl={6}>
                    <Field label="Motivo de Solicitud" name="motivo_solicitud_id" component={SelectBase} items={motivosList} />
                  </Grid>
                  <Grid item xs={6} md={6} xl={6}>
                    <Field label="Cento de Costos" name="centro_costo_id" component={SelectBase} items={centrosList} />
                  </Grid>
                  <Grid item xs={6} md={6} xl={6}>
                    <Field label="Cantidad" name="cantidad" component={NumberBase} />
                  </Grid>
                  <Grid item xs={6} md={6} xl={6}>
                    <Field label="Estado" name="estado" component={SelectBase} items={estadoList} />
                  </Grid>
                </Grid>
...
PresolpedDetalleForm.propTypes = {
  id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
  presolped_id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
...
export default withApi(withNotification(PresolpedDetalleForm));
